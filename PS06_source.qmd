...
* There appears to be a fairly strong relationship between `hs` and `income`. This relationship also appears to be linear.
:::
...
mod_full <- lm(income ~ hs + urbanization + hs:urbanization, data = hate_crimes)
kable(get_regression_table(mod_full))
# Intersect when Urbanization is high:
coef(mod_full)[1]
# Intersect when Urbanization is low:
coef(mod_full)[1] + coef(mod_full)[3]
```

* The high urbanization regression line has the larger intersect.
:::
...
* The slopes do look fairly similar for the two levels of urbanization.
:::
...
* Based on the data visualization in the figure and our answer to question 3, I think it is a better idea to use the simpler, parallel slopes model. 

```{r}
# Type your code and comments inside the code chunk
lm(income ~ hs + urbanization, data = hate_crimes)
```
:::
...
ggplot(hate_crimes, aes(x = urbanization, y = income)) +
  ...
  theme_bw() +
  labs(x = "Urbanization Level", y = "Median household income in USD $", title = "Median ousehold income and Urbanization")
```
:::
...
kable(get_regression_table(med_income_model))
```

```{r}
# Type your code and comments here
ggplot(data = med_income_model, 
       aes(y = income, x = hs, color = urbanization)) +
  geom_point()+ 
  ...
  labs(
    x = "Percent of adults with high-school degree", 
    y = "Median household income in USD $", 
    title = "Parallel Slopes: Income versus education in states with 
    differing levels of urbanization") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
* The intercept for the states with "low" and "high" levels of urbanization are not the same, while the slopes are the same.
:::
...
# Type your code and comments inside the code chunk
med_income_model
coef(med_income_model)[2]
coef(med_income_model)[1]
coef(med_income_model)[1] + coef(med_income_model)[3]
```

* The slope for the regression line with a "high" level of urbanization $`r round(coef(med_income_model)[2], digits = 2)`.

* The intercept for the regression line with a "high" level of urbanization $(113725.20).
:::
...
* The slope for the regression line with a "low" level of urbanization $`r round(coef(med_income_model)[2], digits = 2)`.

* The intercept for the regression line with a "low" level of urbanization $(121058.50).
:::
...
* The median household income is greater in states that have a higher level of urbanization. The median income is higher by about  $`r -1*round(coef(med_income_model)[3], digits = 2)`
:::
...
* For every 1 percentage point increase of high-school educated adults, there is an associated increase in median income of $`r round(coef(med_income_model)[2], digits = 2)`.
:::
...
- $\widehat{\text{income}} = `r comma(coef(summary(med_income_model))[1])` + `r round(coef(summary(med_income_model))[2],2)` \times \text{hs}`r round(coef(summary(med_income_model))[3],2)` \times (0)$
:::
...
```{r}
# Type your code and comments inside the code chunk
predict(med_income_model, newdata = data.frame(urbanization = "high", hs = 85))
```

* I would predict the median household income to be $55152.28.
:::
...
```{r}
# Type your code and comments inside the code chunk
predict(med_income_model, newdata = data.frame(urbanization = "low", hs = 85))
```

* I would predict the median household income to be $47818.96.
:::
...
* It would not be appropriate to predict the median household income with a "low" level of urbanization and a 30% high school graduation percentage. This is because that would be extrapolation, and the predicted value is negative, which doesn't make sense.
:::
...
hate_crimes |>
  filter(state == "Maine") |>
  select(state, income, hs, urbanization) |>
  kable()
# Prediction for Maine
predict(med_income_model, newdata = data.frame(urbanization = "low", hs = 90))
```

* The observed income value for Maine is $51710.

* The models prediction for a state like Maine is $57752.93.

* The residual for this data point is $(`r -(51710 - 57752.93)`).

* Based on the residual, we can see that our model has overestimated the income value for Maine.
:::
...
voles_mod
ggplot(voles_mod, aes(x = veg, y = voles, color = soil)) +
  geom_point() +
  geom_parallel_slopes(method = "lm", se = FALSE) + 
  labs(
    x = "Percent cover of vegetation", 
    y = "Number of Voles", 
    title = "Parallel Slopes: Number of Voles versus vegetation count based on soil humidity") + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
:::
...
* I would recommend to a manager to protect sites with a high percent of vegetation cover because there would most likely be more Voles there.
:::
...
* To have around 30 voles on this sight, the vegetation coverage percentage would have to be around 56.12%.

```{r}
# Type your code and comments inside the code chunk
(30 - coef(voles_mod)[1]) / (coef(voles_mod)[2])
```
:::
...
```{r}
# Type your code and comments inside the code chunk
predict(voles_mod, newdata = data.frame(soil = "moist", veg = 40))

```

* I would predict the possible vole population the site would be able to support is around 34 or 35 voles.
:::
